...
fn sampleMarchingSquares_outline(p: vec2f, cellUv: vec2f, a: vec2f, b: vec2f, straight: bool) -> f32 {
    let lineStrength = (straight ? 1.3333 : 1.0);
    let pa = p - a;
    let ba = b - a;
    let line = pa - ba * dot(pa, ba) / dot(ba, ba);
    return 1.0 - step(lineStrength, length(line));
}

fn sampleMarchingSquares_tile(p: vec2f, cellUv: vec2f, a: vec2f, b: vec2f, tile: i32) -> f32 {
    let pa = p - a;
    let ba = b - a;
    let line = pa - ba * dot(pa, ba) / dot(ba, ba);
    if (tile == 0)
        ...
        let shape12_0 = 1.0 - step(0.35, (rotate2d(PI * 0.25) * cellUv).x);
        let shape12_1 = step(0.35, (rotate2d(PI * -0.75) * cellUv).x);
        return shape12_0 - shape12_1;
    }
    else if (tile == 13) {
        let shape13_0 = step(0.35, (rotate2d(PI * -0.25) * cellUv).x);
        let shape13_1 = step(1.05, (rotate2d(PI * -0.25) * cellUv).x);
...
fn sampleMarchinSquares(tex: SAMPLER_TYPE, uv: vec2f, resolution: vec2f, cellSize: f32, threshold: f32) -> vec2f {
    ...
    let cellIdy = floor(uv.y * gridY);
    vec2 gridUV = vec2f((cellIdx + 0.5) * (1.0 / gridX),
                       (cellIdy + 0.5) * (1.0 / gridY));
    vec2 gridUVCells = vec2f(fract(uv.x * gridX),
                            fract(uv.y * gridY));

    let pixelSize = 1.0 / resolution;
    let gridStep = pixelSize * (cellSize * 0.5);

    let p_bl = gridUV + vec2f(-gridStep.x, +gridStep.y);
    let p_br = gridUV + vec2f(+gridStep.x, +gridStep.y);
    let p_tr = gridUV + vec2f(+gridStep.x, -gridStep.y);
    let p_tl = gridUV + vec2f(-gridStep.x, -gridStep.y);

    let v_bl = SAMPLEMARCHINGSQUARES_SAMPLE_FNC(tex, p_bl);
    let v_br = SAMPLEMARCHINGSQUARES_SAMPLE_FNC(tex, p_br);
    let v_tl = SAMPLEMARCHINGSQUARES_SAMPLE_FNC(tex, p_tl);
    let v_tr = SAMPLEMARCHINGSQUARES_SAMPLE_FNC(tex, p_tr);
