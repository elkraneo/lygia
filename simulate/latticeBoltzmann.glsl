...
vec4 latticeBoltzmannPrevPosSampler(SAMPLER_TYPE tex, vec2 st, vec2 pixel) { 
    vec2 offset = LATTICEBOLTZMANN_SAMPLER_FNC(tex, st).xy;
    return LATTICEBOLTZMANN_SAMPLER_FNC(tex, st - offset * pixel); 
}

vec4 latticeBoltzmann(SAMPLER_TYPE tex, vec2 st, vec2 pixel) {
...
vec4 latticeBoltzmann(SAMPLER_TYPE tex, vec2 st, vec2 pixel, vec2 force) {
